...
This repository contains the fMRI data processing pipeline utilized by the YaleNeuroConnect study. The shell scripts provided will only function properly if you have installed [FSL (6.0.1)](https://fsl.fmrib.ox.ac.uk/fsl/docs/), [BioImage Suite (35)](https://www.nitrc.org/frs/?group_id=51), and [SPM12](https://www.fil.ion.ucl.ac.uk/spm/software/spm12/), and have each of those softwares added to the 'softwares' folder in this repository. You will also need to download [optiBET](https://montilab.psych.ucla.edu/fmri-wiki/optibet/#:~:text=doi:10.1371/journal.pone,both%20FSL%20and%20AFNI%20installed.) and add that to the softwares folder. These scripts are located in the '/sample/scripts/' folder provided in this repository. To use this processing pipeline on the data provided on OpenNeuro, download the raw BIDS data files and move the subjects' folders into the '/sample/sourcedata/' folder in your repository's fork. 
...
1. strip_brain.csh -- runs skull stripping by calling on to FSL optimized brain extraction tool (optiBET) to skull strip the T1 weighted MPRAGE.
...
2. batch_bids_unzip_motion_zip.csh -- performs motion correction along 6 dimensions (24 parameters total) by calling SPM12 functions.  
...
3. Non-linear registration of the individuals' structural MRIs to a template brain.
...
4. bislinearregister.csh -- performs linear registration of the functional scans to the structural MRIs.  
...
5. create_xmlgfiles_matrix_byrun.csh -- creates the setup files for making the functional connectivity matrices.  
...
7. batch_uniformsmoothing.csh -- uniform smoothing using a 6mm FWHM Gaussian kernel.
