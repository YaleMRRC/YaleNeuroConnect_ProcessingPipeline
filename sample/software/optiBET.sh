...
export FSLDIR=/data1/software/fsl-6.0.1/fsl/
...
#######Dustin Addition
####### STEP 0 MAKE SURE IMAGES ARE RAS
#fslswapdim ${iopt} RL AP SI ${i}_RAS.nii.gz
fslreorient2std  ${iopt} ${i}_RAS.nii.gz
...
if [[ "$s1" == "bet" ]]; then
    ...
    bet ${i}_RAS.nii.gz ${i}_step1 -B -f 0.1
else
...
#dustin edit
#sh /data1/software/convert_fslmat.sh ${i}_step2_tmp.mat > ${i}_step2.mat
#rm ${i}_step2_tmp.mat
#dustin edit
...
fnirt --in=${i}_RAS.nii.gz --aff=${i}_step2.mat --cout=${i}_step3 --config=T1_2_MNI152_2mm
...
applywarp --ref=${FSLDIR}/data/standard/MNI152_T1_2mm --in=${i}_RAS.nii.gz --warp=${i}_step3 --out=${i}_step4
...
applywarp --ref=${i}_RAS.nii.gz --in=${FSLDIR}/data/standard/${mask} --warp=${i}_step5.nii.gz --out=${i}_step6 --interp=nn
...
#######Dustin Addition
#######Fill in hole found in brain mask
#######original brain_mask has venticles masked out

echo "step 7a filling in holes for subject ${i}"
fslmaths ${i}_optiBET_brain_mask.nii.gz -fillh  ${i}_optiBET_brain_mask2
...
#######Dustin Addition
#######Use new mask with holes filled in
fslmaths ${i}_RAS.nii.gz -mas ${i}_optiBET_brain_mask2 ${i}_optiBET_brain

#######Dustin Addition
#######Put everything back into LPS
fslswapdim ${i}_optiBET_brain.nii.gz LR PA SI ${i}_optiBET_brain.nii.gz
fslswapdim ${i}_optiBET_brain_mask2.nii.gz LR PA SI ${i}_optiBET_brain_mask2.nii.gz
fslswapdim ${i}_optiBET_brain_mask.nii.gz LR PA SI ${i}_optiBET_brain_mask.nii.gz

#######Dustin Addition
#######Make weight image
fslmaths ${i}_optiBET_brain_mask.nii.gz  -mul 100 ${i}_optiBET_brain_weight.nii.gz ; fslmaths ${i}_optiBET_brain_weight.nii.gz -s 2.6082 ${i}_optiBET_brain_weight.nii.gz
...
rm ${i}_step1.nii.gz ${i}_step1_mask.nii.gz ${i}_step2.nii.gz ${i}_step2.mat ${i}_step3.nii.gz ${i}_step4.nii.gz ${i}_step5.nii.gz ${i}_step6.nii.gz ${i}_RAS_to_MNI152_T1_2mm.log  ${i}_RAS.nii.gz
fi
